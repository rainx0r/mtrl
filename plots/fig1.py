"""Opening Figure: Bar chart for parameter matching"""
# TODO: Fix
...
def plot_results(
    mt10_results, mt50_results, param_counts_mt10, param_counts_mt50, mtrl_archs
):
    ...
    for i, (name, data) in enumerate(mt10_results.items()):
        ...
        iqm = scipy.stats.trim_mean(
            data, proportiontocut=0.25, axis=None
    ...
    ax1.set_xticklabels(
        [f"{p/1e6:.1f}M" for p in list(param_counts_mt10)], fontsize="x-large"
    )
...
def plot_learning_curves(
    ...
    param_counts = sorted(
        list(simple_results.keys()) + [params for params, _, _ in mtrl_results.values()]
    )
    ...
    ax1.set_xticklabels(
        [f"{p/1e6:.1f}M" for p in list(params_dict.keys())], fontsize="x-large"
    )
    ...
    param_counts = sorted(
        list(mt50_simple.keys()) + [params for params, _, _ in mt50_mtrl.values()]
    )
    ...
    simple_results_std_mt50 = {
        param_counts[0]: np.std(mt50["SAC"]),
        param_counts[1]: np.std(mt50["SM Params"]),
        param_counts[2]: np.std(mt50["MOORE Params"]),
        param_counts[3]: np.std(mt50["PaCo Params"]),
        param_counts[4]: np.std(mt50["2048"]),
        param_counts[5]: np.std(mt50["4096"]),
    }
    ...
    ax2.plot(
        list(params_dict_mt50.values()), values, "-o", color="blue", label="Simple FF"
    )
    ax2.fill_between(
        list(params_dict_mt50.values()),
        np.array(values) - np.array(errors),
        np.array(values) + np.array(errors),
        alpha=0.2,
        color="blue",
    ...
    ax2.set_xticklabels(
        [f"{p/1e6:.1f}M" for p in list(params_dict_mt50.keys())], fontsize="x-large"
    )
...
def main():
    ...
    mt50_results = {
        "mt50_mtmhsac_v2": [0.524, 0.54, 0.014000000000000002, 0.5472, 0.5472],
    ...
    mt50_results = {
        "mt50_mtmhsac_v2": [0.524, 0.54, 0.014000000000000002, 0.5472, 0.5472],
        "mt50_softmodules_task_weights_false": [0.638, 0.6688, 0.5924, 0.3972, 0.5328],
        "mt50_paco": [0.6888, 0.5864, 0.5943999999999999, 0.5568000000000001, 0.5808],
        "mt50_moore": [
            0.6507999999999999,
            0.6476000000000001,
            0.6576,
        ...
        "mt50_mtmhsac_v2_2048_width": [0.7816, 0.8708000000000001, 0.8, 0.7456, 0.8608],
        "mt50_mtmhsac_v2_4096_width": [
            0.7611999999999999,
            0.8640000000000001,
            0.8032,
    ...
    mtrl_results = {
        "SM": (
            param_counts_mt10[1],
            scipy.stats.trim_mean(mt10["SM"], proportiontocut=0.25, axis=None),
            np.std(mt10["SM"]),
        ),
        "MOORE": (
            param_counts_mt10[3],
            scipy.stats.trim_mean(mt10["MOORE"], proportiontocut=0.25, axis=None),
            np.std(mt10["MOORE"]),
        ),
        "PaCo": (
            param_counts_mt10[5],
            scipy.stats.trim_mean(mt10["PaCo"], proportiontocut=0.25, axis=None),
            np.std(mt10["PaCo"]),
        ),
    }
    ...
    simple_results = {
        param_counts_mt10[0]: scipy.stats.trim_mean(
            mt10["SAC"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt10[1]: scipy.stats.trim_mean(
            mt10["SM Params"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt10[3]: scipy.stats.trim_mean(
            mt10["MOORE Params"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt10[5]: scipy.stats.trim_mean(
            mt10["PaCo Params"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt10[7]: scipy.stats.trim_mean(
            mt10["1024"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt10[8]: scipy.stats.trim_mean(
            mt10["2048"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt10[9]: scipy.stats.trim_mean(
            mt10["4096"], proportiontocut=0.25, axis=None
        ),
    }
    ...
    mt50_mtrl = {
        "SM": (
            param_counts_mt50[1],
            scipy.stats.trim_mean(mt50["SM"], proportiontocut=0.25, axis=None),
            np.std(mt50["SM"]),
        ),
        "MOORE": (
            param_counts_mt50[3],
            scipy.stats.trim_mean(mt50["MOORE"], proportiontocut=0.25, axis=None),
            np.std(mt50["MOORE"]),
        ),
        "PaCo": (
            param_counts_mt50[5],
            scipy.stats.trim_mean(mt50["PaCo"], proportiontocut=0.25, axis=None),
            np.std(mt50["PaCo"]),
        ),
    }

    simple_results_mt50 = {
        param_counts_mt50[0]: scipy.stats.trim_mean(
            mt50["SAC"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt50[1]: scipy.stats.trim_mean(
            mt50["SM Params"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt50[3]: scipy.stats.trim_mean(
            mt50["MOORE Params"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt50[5]: scipy.stats.trim_mean(
            mt50["PaCo Params"], proportiontocut=0.25, axis=None
        ),
        # param_counts_mt50[7]: scipy.stats.trim_mean(mt50['1024'], proportiontocut=0.25, axis=None),
        param_counts_mt50[7]: scipy.stats.trim_mean(
            mt50["2048"], proportiontocut=0.25, axis=None
        ),
        param_counts_mt50[8]: scipy.stats.trim_mean(
            mt50["4096"], proportiontocut=0.25, axis=None
        ),
    }
